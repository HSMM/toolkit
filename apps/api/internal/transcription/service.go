// Package transcription реализует поток транскрибации:
// загрузка аудио → MinIO → запись в БД → постановка job → воркер шлёт в
// GigaAM, опрашивает, сохраняет сегменты.
//
// Используется для пользовательских загрузок (kind='upload'), а также
// доменными модулями (звонки, встречи) через тот же Service-API.
