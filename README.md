...
- ✅ **Telegram-мессенджер** — MTProto user-client внутри Toolkit: настройки админа, QR-подключение аккаунта, личные чаты и группы, поиск по чатам, чтение/отправка сообщений, просмотр и отправка вложений (фото, видео, аудио, документы), realtime updates через `telegram-worker` → API → WebSocket, cache retention 180 дней.
