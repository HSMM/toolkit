...
Софтфон, видеоконференции, Telegram-мессенджер и автоматическая транскрибация — в одной вкладке браузера, под одним логином корпоративного Bitrix24, с историей и поиском по всему, что произошло.
...
| **Мессенджеры** | Telegram как пользовательский MTProto-клиент внутри Toolkit: QR-подключение аккаунта, личные чаты и группы, поиск по списку чатов, чтение и отправка сообщений, realtime-обновления через worker + WebSocket, локальный cache с retention 180 дней |
...
- ✅ **Telegram-мессенджер** — MTProto user-client внутри Toolkit: настройки админа, QR-подключение аккаунта, личные чаты и группы, поиск по чатам, чтение/отправка сообщений, realtime updates через `telegram-worker` → API → WebSocket, cache retention 180 дней.
...
- ❌ **Telegram attachments и расширенный auth** — чтение/отправка текста работает; загрузка файлов, скачивание вложений, вход по номеру/коду/2FA, каналы, секретные чаты и привязка Telegram-чата к CRM-контакту/сделке вынесены в следующий этап.
...
| **E9** Мессенджеры | Telegram user-client: GramJS worker, QR-login, хранение зашифрованной MTProto-сессии, sync личных чатов и групп, cache 500 сообщений на чат с retention 180 дней, отправка текста, WebSocket realtime, Telegram-like UI и поиск по чатам | Готово для MVP |
