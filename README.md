...
| **Мессенджеры** | Telegram как пользовательский MTProto-клиент внутри Toolkit: QR-подключение аккаунта, личные чаты и группы, поиск по списку чатов, чтение и отправка сообщений, просмотр и отправка фото/видео/аудио/файлов, realtime-обновления через worker + WebSocket, локальный cache с retention 180 дней |
