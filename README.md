...
- ✅ **Авторизация** — OAuth Bitrix24 и локальный вход по email/password, восстановление пароля через SMTP-письмо, JWT (HS256, 15 мин) + refresh-сессии, RBAC (admin / user / контекстная manager). Пароль пользователю задаёт администратор; вход через Bitrix24 остаётся доступным.
...
| **E1** Авторизация | OAuth Bitrix24, локальный вход по email/password, восстановление пароля, JWT-сессии (HS256, 15 мин) + refresh, RBAC | Готово |
