...
- отображение входящих вложений как карточек в thread;
- скачивание входящих вложений через backend;
...
```json
{
  ...
  "text": "Здравствуйте",
  "attachment_ids": ["uploaded-attachment-uuid"]
}
```
...
### Загрузить Вложение Для Отправки

`POST /api/v1/messenger/telegram/attachments/upload`
...
Загруженный файл хранится во временном Toolkit storage и прикрепляется к сообщению через `attachment_ids`. Если сообщение не отправлено, временный файл удаляется retention-job.
...
### MVP v1. Этап 5. Входящие Вложения
...
- 2FA password step;
- upload endpoint для отправки вложений;
- отправка вложений через MTProto;
...
- пользователь видит и скачивает входящие вложения;
...
- пользователь может отправлять вложения;
